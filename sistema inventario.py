...
def registrar_venta(pedido):
    total_venta = sum(inventario[producto]['precio_venta'] * cantidad for producto, cantidad in pedido.items())
    for producto, cantidad in pedido.items():
        inventario[producto]['cantidad'] -= cantidad
    print(f"Venta registrada exitosamente. Total de venta: ${total_venta:.2f}")
